            ...
            <div class="row">
                <div class="col-lg-12">
                    @if (Auth::guard('admin')->user()->can('brand.create'))
                        <div class="mt-2 mb-4">
                            <a href="{{ url('admin/brand/add') }}" class="btn btn-sm btn-primary text-white pull-right"><i
                                ...
                                <tbody>
                                    @foreach ($brands as $brand)
                                        <tr>
                                            ...
                                            <td>{{ ucwords($brand->slug) }}</td>
                                            <td>{{ ucwords($brand->meta_title) }}</td>
                                            <td>{{ ucwords($brand->meta_description) }}</td>
                                            <td>{{ ucwords($brand->meta_keywords) }}</td>
                                            <td>{{ ucwords($brand->created_by_name) }}</td>
                                            <td>
                                                @if ($brand->status == 0)
                                            ...
                                            <td>
                                                @if (Auth::guard('admin')->user()->can('brand.edit'))
                                                    <a href="{{ url('admin/brand/edit/' . $brand->id) }}"
                                                        ...
                                                        title="Edit brand">Edit</a>
                                                @endif
                                                @if (Auth::guard('admin')->user()->can('brand.delete'))
                                                    <a class="btn btn-sm btn-danger text-white" id=""
                                                        data-id="{{ $brand->id }}" data-token={{ csrf_token() }}
                                                        data-href="{{ url('admin/brand/delete/' . $brand->id) }}"
                                                        data-formId="delete-form-{{ $brand->id }}"
                                                        onclick="confirmDelete(this)" title="Delete brand">
                                                        Delete
                                                    </a>
                                                @endif
